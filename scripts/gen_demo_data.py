...
Usage (see scripts/seed-demo-data.sh for the wrapper that applies these):
    ...
    python3 scripts/gen_demo_data.py --owner-id <cognito-sub> --certs 40 \
        --accounts 15 --audit-events 30 --out-dir /tmp/seed
"""
import argparse
import hashlib
import json
...
# other Status is invisible to the alerting path, so most rows use these and a
# minority use the others deliberately, to prove the filter works.
CERT_ALERTABLE_STATUS = "ISSUED"
CERT_OTHER_STATUSES = ("EXPIRED", "PENDING_VALIDATION", "REVOKED")
...
# The three bands the UI colours, as (min_days, max_days). Generation guarantees
# coverage of each rather than leaving it to chance, so seeding exercises the
# rendering and not just the query.
BANDS = (
    ("red", 1, 7),      # <= 7 days
    ("amber", 8, 30),   # <= 30 days
    ("green", 31, 400),
...
def _days_out(rng, index, total):
    """Spread expiry across all three bands, guaranteeing each is represented."""
    if index < len(BANDS):
        _, low, high = BANDS[index]  # first N rows: one per band
    else:
        _, low, high = BANDS[index % len(BANDS)]
...
def build_certs(owner_id, count, rng):
    ...
    for i in range(count):
        ...
        # Roughly 1 in 6 rows gets a non-alertable status, so the ExpiryIndex
        # filter has something to exclude.
        status = (
            rng.choice(CERT_OTHER_STATUSES)
            if i % 6 == 5
            else CERT_ALERTABLE_STATUS
        )
        ...
        items.append(
            {
                ...
                # OwnerIndex HASH. Must be the caller's Cognito sub or GET /certs
                # will never return the row.
                "OwnerId": {"S": owner_id},
                "Domain": {"S": domain},
...
def main():
    ...
    parser.add_argument("--owner-id", required=True,
                        help="Cognito sub that will own these rows (OwnerIndex HASH key)")
