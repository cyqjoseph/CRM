# Solution Design: Centralised Resource Manager Cross-Environments
Data Sensitivity: Internal (contains High-sensitivity AD rotation logs, tokenized PII) | approved_at: 2025-06-17T00:00:00Z
...
sequenceDiagram
    ...
    participant DiscoveryLambda
    participant ACM
    participant FargateTask
    participant ADServer
    participant CertTable
    participant ADTable
    participant AuditTable
    EventBridge->>StepFunctions: "start discovery workflow"
    StepFunctions->>DiscoveryLambda: "invoke ACM/Secrets/IAM scan"
    DiscoveryLambda->>ACM: "DescribeCertificate"
    ACM-->>DiscoveryLambda: "cert metadata"
    DiscoveryLambda->>CertTable: "PutItem cert metadata"
    StepFunctions->>FargateTask: "RunTask AD discovery"
    FargateTask->>ADServer: "LDAP/ADWS query"
    ADServer-->>FargateTask: "account metadata"
    FargateTask->>ADTable: "PutItem account metadata"
    FargateTask-->>StepFunctions: "task result"
    StepFunctions->>AuditTable: "PutItem discovery event"
```
...
sequenceDiagram
    autonumber
    participant EventBridge
    participant ExpiryLambda
    participant CertTable
    participant SNS
    participant SQS
    participant JiraLambda
    participant Jira
    participant DLQ
    participant AuditTable
    EventBridge->>ExpiryLambda: "hourly expiry check"
    ExpiryLambda->>CertTable: "Query ExpiryIndex threshold"
    CertTable-->>ExpiryLambda: "matching items"
    ExpiryLambda->>SNS: "publish alert"
    ExpiryLambda-)SQS: "send ticket request"
    SQS->>JiraLambda: "poll message"
    alt "ticket creation succeeds"
        JiraLambda->>Jira: "POST create issue"
        Jira-->>JiraLambda: "issue id"
        JiraLambda->>AuditTable: "PutItem ticket event"
    else "ticket creation fails"
        JiraLambda->>DLQ: "move to DLQ"
    end
    ExpiryLambda->>AuditTable: "PutItem alert event"
```
...
sequenceDiagram
    ...
    participant AuditTable
    participant SNS
    User->>APIGW: "POST /certs/certId/renew"
    APIGW->>APILambda: "authorize and invoke"
    APILambda->>StepFunctions: "StartExecution renewal workflow"
    StepFunctions-->>APILambda: "executionArn"
    APILambda->>AuditTable: "PutItem manual trigger event"
    APILambda-->>APIGW: "202 executionArn requestId"
    APIGW-->>User: "202 accepted"
    StepFunctions->>CertTable: "UpdateItem renewal result"
    StepFunctions-)SNS: "publish completion notification"
    User->>APIGW: "GET /executions/executionId"
    APIGW->>APILambda: "authorize and invoke"
    APILambda-->>User: "status response"
```
...
|----|----------|---------|-----------------|
| OED-01 | Hot audit table TTL window before S3 export | 30 days / 90 days / 180 days | 90 days — balances hot-retrieval SLA (NFR-07) with table size/cost |
| OED-02 | UI status-check mechanism for async triggers | Client polling only / polling + SNS-to-UI push (requires an additional service not yet approved) | Polling only, using exponential backoff — no new service required |
| OED-03 | Self-signed/on-prem (non-AD) certificate intake | Manual UI entry only / extend Fargate scan scope | Manual UI entry — architecture scopes automated discovery to ACM and AD only |
| OED-04 | Cross-owner sharing of a resource (multiple owners per cert/account) | Single OwnerId attribute / OwnerIds list with fan-out GSI writes | Single OwnerId for v1; revisit if BRD requires multi-owner delegation |
